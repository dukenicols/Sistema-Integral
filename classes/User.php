<?php
	class User {
		...
		$admin = $this->is_admin();
			...
			$viewer = $this->is_viewer();
			if($viewer)
			{
				return 'viewer';
			}
			$executive = $this->is_executive();
			if($executive)
			{
				return 'executive';				
			}
			$approver = $this->is_approver();
			if($approver)
			{
				return 'approver';
			}
		...
		{
			 return ($this->_data->userGroup == '1') ? true :  false;
		}
		private function is_viewer()
		{
			return ($this->_data->userGroup == '2') ? true :  false;
		}
		private function is_executive()
		{
			return ($this->_data->userGroup == '3') ? true :  false;
		}
		private function is_approver()
		{
			return ($this->_data->userGroup == '4') ? true :  false;
		}
